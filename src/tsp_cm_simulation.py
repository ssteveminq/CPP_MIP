...
def set_sequential_goal_tsp(cluster_goals, tsp_path, best_idx):
    #best_idx - 0, 1, 2(-2), 3(-1)
    if best_idx>3 or best_idx<0:
        print("input_idx is wrongwrong")
        return None
    else:
        goal_tsp=[]
        if best_idx==0:
            for j in range(len(tsp_path)):
                goal_tsp.append(cluster_goals[tsp_path[j]])
        elif best_idx==1:
            for j in range(1, len(tsp_path)):
                goal_tsp.append(cluster_goals[tsp_path[j]])
            goal_tsp.append(cluster_goals[tsp_path[0]])
        elif best_idx==2:
            goal_tsp.append(cluster_goals[tsp_path[-2]])
            goal_tsp.append(cluster_goals[tsp_path[-1]])
            for j in range(len(tsp_path)-2):
                goal_tsp.append(cluster_goals[tsp_path[j]])
        elif best_idx==3:
            goal_tsp.append(cluster_goals[tsp_path[-1]])
            for j in range(len(tsp_path)-1):
                goal_tsp.append(cluster_goals[tsp_path[j]])

        return goal_tsp

def set_sequential_goal_tsp2(cluster_goals, tsp_path, best_idx):
    #best_idx - 0, 1, 2(-2), 3(-1)
    if best_idx>2 or best_idx<0:
        print("input_idx is wrongwrong")
        return None
    else:
...
def calc_IG_Point(cur_pos, goalcandidates, entropy_map, params_global, robot_param):
    weight_entropy=1.0
    weight_travel=20.0
    print("goalcandidates", goalcandidates)
    isIGzero=False
    if len(goalcandidates)>0:
        igs=[]
        considered_trjs=[]
        for j, trj in enumerate(goalcandidates):
            ig=0
            travel=0
            ref_x = cur_pos[0]
            ref_y = cur_pos[1]
            ig=get_expected_entropy_point(goalcandidates[j], entropy_map, params_global, robot_param)
            dx = goalcandidates[j][0]-ref_x
            dy = goalcandidates[j][1]-ref_y
        ...
        return goalcandidates[max_idx], max_idx, isIGzero
    else:
...
def calc_IG_trjs_hierarchy( trj_candidates, connect_counts, entropy_map, params_global, trjs, agentnum, robot_param,  horizon=30):
    ...
    if len(trj_candidates)>0:
        ...
        sorted_ig = sorted(((v,i) for i, v in enumerate(igs)),reverse=True)
        max_idx = sorted_ig[0][1]
        return trj_candidates[max_idx], max_idx
    else:
        print("no trajectory candidates")
        return None
...
def get_expected_entropy_point(pos, entropy_map, params_searchmap, robot_param):
    center_x=pos[0]
    center_y=pos[1]
...
def generating_globaltrj_Astar(cur_state, AStarplanner, goal):

    sx = cur_state[0]
    sy = cur_state[1]
    rx,ry, success=AStarplanner.planning(sx,sy,goal[0],goal[1])
    if success:
        sp=Spline2D(rx,ry)
        return sp
    else:
...
if __name__ == "__main__":
    ...
    # tspmanager = tsp_manager(1,goal_test,instates)
    ...
    for _ in range(params.numiters):
        ...
        #check one of the agent reaches to to it's goal
        goal_reached = False
        for i in range(num_agent):
            if goal_dist[i]< params.goal_tol:
                goal_reached =True

        # agentgoal_reached = [False for i in range(num_agent)]
        #should update goal_poses[i] for each agent
        if Istsp_path:
            agentgoal_reached = [False for i in range(num_agent)]
            for i in range(num_agent):
                if goal_dist[i]< params.goal_tol:
                    agentgoal_reached[i]=True
                    ...
                    offset_idx=1
                    IGzero=False
                    while (cur_path_idx+offset_idx)<len(goal_tsps[i]):
                        tempgoals=[]
                        for k in range(cur_path_idx, cur_path_idx+offset_idx):
                            tempgoals.append(goal_tsps[i][k])
                        goal, tidx, IGzero = calc_IG_Point(states[i], tempgoals, pmap_global, params_globalmap, robot_params[i])
                        if IGzero==True:
                            break;
                        else:
                            offset_idx=offset_idx+1
                    print("len(goal_tsp_i)", len(goal_tsps[i]))
                    cur_path_idx=cur_path_idx+tidx
            ...
            if abs(prev_entropy-curentropy)>entropy_difference*initial_entropy:
                Istsp_path=False
        ...
        if goal_reached and Istsp_path==False:                                          # goal is reached
            ...
            #calculate the position of center--cluster
            ...
            for i in range(num_agent):
                ...
                #which one should we first?
                # sample_goalset =filtering_goals(goal_tsp, goal_poses, i)
                # gtrjs= generating_globaltrjs_Astar(states[i], a_star,sample_goalset) 
                # sp_gtrjs = trjs_to_sample(gtrjs,robot_params[i])
                # trjs_candidate =[]
                # for gtrj in sp_gtrjs:
        ...
        if show_animation:
            ...
            for i in range(num_agent):
                ...
                axes[0,1].scatter(goal_poses[i][0], goal_poses[i][1], facecolor=ColorSet[i],edgecolor=ColorSet[i])
        ...
        if iter==1: #FixMe: it was 1
            ...
            for i in range(num_agent):
                ...
                connect_count=calc_connectivity_utility(trjs_candidateset[i], sample_goalset, horizon)
                best_trj, idx = calc_IG_trjs_hierarchy(trjs_candidateset[i], connect_count, pmap_global,  params_globalmap, best_trjs, i, robot_params[i], horizon )
                goal = choose_goal_from_trj(best_trj, params_globalmap, horizon)
                # goals.append(goal)
                goal_xs[i]=goal[0]
                goal_ys[i]=goal[1]
